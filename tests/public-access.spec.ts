...
test("la page légale est accessible depuis l'application", async ({ page }) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.setItem("stick-map-show-presentation", "false"));
  await page.reload();
  ...
  await expect(
    page.getByRole("heading", { name: /Ultra Stick Map — Informations légales/i }),
  ).toBeVisible();
});
...
test("le classement public est accessible", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Tout le monde" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Amis" })).toBeVisible();
